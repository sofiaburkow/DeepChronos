...
def prepare_features(df, feature_list, ip_encoding="none"):
    '''
    ...
    df = df[feature_list + ip_feature_cols]
...
def save_data(X_train, y_train, y_phase_train, X_test, y_test, y_phase_test, pipeline, numeric_cols, categorical_cols, ip_encoding, output_dir, sparse=True):
    '''
...
def preprocess_data(df_train, df_test, features_list, ip_encoding, output_dir, label_name="attack", save=True):
    '''
    Preprocess data: prepare features, construct pipeline, transform data, and save to disk.
    '''
    # Filter on features
    df_train_features, numeric_cols, categorical_cols, _ = prepare_features(
        df_train, features_list, ip_encoding=ip_encoding,     
    )
    df_test_features, _, _, _ = prepare_features(
        df_test, features_list, ip_encoding=ip_encoding,
    )
    ...
    if save:
        save_data(X_train, y_train, y_phase_train, X_test, y_test, y_phase_test, pipeline, numeric_cols, categorical_cols, ip_encoding, output_dir)
...
if __name__ == "__main__":
    ...
    # Prepare features
    df_train_features, numeric_cols, categorical_cols, ip_feature_cols = prepare_features(

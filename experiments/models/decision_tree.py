...
def train_and_test_decision_tree(dataset_dir):
    ...
    output_dir = f"decision_tree/{parts[-4]}/{parts[-3]}/{parts[-2]}"
    plot_misclassified_samples(y_test, y_pred, y_phase_test, output_dir)


if __name__ == "__main__":
    # Command: uv run python experiments/models/decision_tree.py <dataset_dir>
    
    if len(sys.argv) < 2:
        print("Usage: python experiments/models/decision_tree.py <dataset_dir>")
        sys.exit(1)
